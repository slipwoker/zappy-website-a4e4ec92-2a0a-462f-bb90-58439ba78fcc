  ...
  if (contactForm) {
    ...
    function getFormFields() {
      return Array.from(contactForm.querySelectorAll(
        'input:not([type="submit"]):not([type="reset"]):not([type="button"]):not([type="hidden"]), textarea, select'
      ));
    }
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target.closest('input, textarea, select');
      if (field && field.classList.contains('field--error') && contactForm.contains(field)) {
        validateField(field);
      }
    });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Soundent / Soundmir משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'a4e4ec92-2a0a-462f-bb90-58439ba78fcc',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'a4e4ec92-2a0a-462f-bb90-58439ba78fcc';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'a4e4ec92-2a0a-462f-bb90-58439ba78fcc';
